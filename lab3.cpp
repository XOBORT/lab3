...
int main() {
	...
	file.open("Omsk.csv");
	if (!file.is_open())
		cout << "ne ydalos otkrit file" << endl;
	else {
		while (!file.eof())
		{
			getline(file, line1);
	...
	double* z = new double[145 - count9];
	...
	vector<int> stroki;
	int nstrok = 0;
	for (int i = 0; i < 145; i++) if (temp[i][month] != 999.9) { temperature.push_back(temp[i][month]); nstrok++; stroki.push_back(nstrok); }
